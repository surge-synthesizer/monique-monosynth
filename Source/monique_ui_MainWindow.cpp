...
public:
ButtonFlasher( Button*button_, bool success_, int num_flashs_ = 2 ) noexcept :
    button(button_), counter(0), button_state( IS_BUTTON_ON( button ) ), success( success_ ), num_flashs( num_flashs_ )
    {
...
void Monique_Ui_Mainwindow::toggle_modulation_slider_top_button( Button*button_ ) noexcept
{
...
void Monique_Ui_Mainwindow::buttonClicked (Button* buttonThatWasClicked)
{
    ...
    else if (buttonThatWasClicked == button_programm_delete)
    {
        ...
        const bool success = synth_data->remove();
