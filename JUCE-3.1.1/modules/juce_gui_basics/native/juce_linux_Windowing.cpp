...
struct Atoms
{
    ...
    static Atom getIfExists (const char* name) {
        return XInternAtom (display, name, True);
    }
    static Atom getCreating (const char* name) {
    ...
    static bool isMimeTypeFile (const Atom atom) {
        return getName (atom).equalsIgnoreCase ("text/uri-list");
    }
};
...
#if JUCE_USE_XSHM
    bool isUsingXShm() const noexcept {
        return usingXShm;
    }
#endif
private:
...
//==============================================================================
    void handleWindowMessage (XEvent& event)
    {
        ...
        case ButtonRelease:
            ...
            break;
        case MotionNotify:
            handleMotionNotifyEvent (event.xmotion);
            break;
        case EnterNotify:
            handleEnterNotifyEvent (event.xcrossing);
            break;
        case LeaveNotify:
            handleLeaveNotifyEvent (event.xcrossing);
            break;
        case FocusIn:
            handleFocusInEvent();
            break;
        ...
        case MappingNotify:
            ...
            break;
        case ClientMessage:
            handleClientMessageEvent (event.xclient, event);
            break;
        case SelectionNotify:
            handleDragAndDropSelection (event);
            break;
        case ConfigureNotify:
            handleConfigureNotifyEvent (event.xconfigure);
            break;
        case ReparentNotify:
        ...
        case SelectionRequest:
            handleExternalSelectionRequest (event);
            break;
...
// Look for a subsequent key-down event with the same timestamp and keycode
    ...
    void handleButtonPressEvent (const XButtonPressedEvent& buttonPressEvent)
    {
        ...
        {
        case Keys::WheelUp:
            handleWheelEvent (buttonPressEvent, 50.0f / 256.0f);
            break;
        case Keys::WheelDown:
            handleWheelEvent (buttonPressEvent, -50.0f / 256.0f);
            break;
        case Keys::LeftButton:
            handleButtonPressEvent (buttonPressEvent, ModifierKeys::leftButtonModifier);
            break;
        case Keys::RightButton:
            handleButtonPressEvent (buttonPressEvent, ModifierKeys::rightButtonModifier);
            break;
        case Keys::MiddleButton:
            handleButtonPressEvent (buttonPressEvent, ModifierKeys::middleButtonModifier);
            break;
        default:
            break;
        }
...
:
        ...
        String textOrFiles;
        const Atom* getMimeTypes() const noexcept {
            return isText ? Atoms::get().externalAllowedTextMimeTypes
                   : Atoms::get().externalAllowedFileMimeTypes;
        }
        int getNumMimeTypes() const noexcept {
            return isText ? numElementsInArray (Atoms::get().externalAllowedTextMimeTypes)
...
void* MouseCursor::createStandardMouseCursor (MouseCursor::StandardCursorType type)
{
    ...
    case NormalCursor:
    case ParentCursor:
        return None; // Use parent cursor
    case NoCursor:
        return CustomMouseCursorInfo (Image (Image::ARGB, 16, 16, true), 0, 0).create();
    case WaitCursor:
        shape = XC_watch;
        break;
    case IBeamCursor:
