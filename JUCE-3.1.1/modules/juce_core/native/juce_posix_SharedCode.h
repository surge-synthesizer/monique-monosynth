...
struct HighResolutionTimer::Pimpl
{
    ...
    {
        // to us
       startMicro(newPeriod*1000);
    }
    
    void startMicro(int newMicroPeriod)
    {
        if (periodMicro != newMicroPeriod)
        {
            if (thread != pthread_self())
            {
                stop();

                periodMicro = newMicroPeriod;
                shouldStop = false;

                if (pthread_create (&thread, nullptr, timerThread, this) == 0)
                    setThreadToRealtime (thread, (uint64) newMicroPeriod);
                else
                    jassertfalse;
            }
            else
...
private:
    ...
    void timerThread()
    {
        ...
        while (! shouldStop)
        {
            ...
            if (lastPeriod != periodMicro)
            {
                lastPeriod = periodMicro;
                clock = Clock (lastPeriod);
            }
        }
    ...
    struct Clock
    {
        ...
        Clock (double micro) noexcept
        {
            mach_timebase_info_data_t timebase;
            (void) mach_timebase_info (&timebase);
            delta = (((uint64_t) (micro * 1000.0)) * timebase.denom) / timebase.numer;
       ...
       #elif JUCE_ANDROID
        Clock (double micro) noexcept  : delta ((uint64) (micro * 1000))
        {
        }
    ...
    static bool setThreadToRealtime (pthread_t thread, uint64 periodMicro)
    {
       #if JUCE_MAC || JUCE_IOS
        thread_time_constraint_policy_data_t policy;
        policy.period      = (uint32_t) (periodMicro * 1000);
