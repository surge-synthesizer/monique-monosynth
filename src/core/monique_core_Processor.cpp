...
class ClockSmoothBuffer : RuntimeListener
{
  ...
  public:
    ...
    inline float get_average() const noexcept { return sum * (1.0 / CLOCKS_TO_SMOOTH); }
  ...
  public:
    //==========================================================================
    COLD ClockSmoothBuffer(RuntimeNotifyer *const notifyer_)
        : RuntimeListener(notifyer_), pos(0), sum(0)
    {
        reset(0);
    }
...
struct MoniqueAudioProcessor::standalone_features : public juce::Timer
{
    ...
    void set_audio_online() noexcept { block_lock.exit(); }
      ...
      AudioProcessor(BusesProperties().withOutput("Output", juce::AudioChannelSet::stereo(), true))
{
    ...
    if (is_standalone())
    {
        ...
        standalone_features_pimpl->clock_smoother =
            std::make_unique<ClockSmoothBuffer>(runtime_notifyer);
    }

    DBG("MONIQUE: init core");
    DBG("MONIQUE: version - " << Monique::Build::FullVersionStr << " built at "
                              << Monique::Build::BuildDate << " " << Monique::Build::BuildTime);
    ...
    // PARAMETER ORDER
    {
        ...
        // REORDER THE PARAMS
        {
                ...
                synth_data->get_atomateable_parameters();
            {
                ...
                for (int j = 0; j < list.pairs.size(); ++j)
                {
                    ...
                    for (int i = 0; i != automateable_parameters.size(); ++i)
                    {
                        ...
                        {
                            reordered_automateable_parameters.add(
                                automateable_parameters.removeAndReturn(i));
                            break;
                        }
                    }
                }
            ...
            {
                String name =
            synth_data->get_atomateable_parameters().getReference(i)->get_info().name; String
            short_name =
            synth_data->get_atomateable_parameters().getReference(i)->get_info().short_name;
...
int MoniqueAudioProcessor::getNumParameters() { return automateable_parameters.size(); }
...
float MoniqueAudioProcessor::getParameter(int i_)
{
    ...
    {
        value =
            (1.0f + automateable_parameters.getUnchecked(i_ - 1)->get_modulation_amount()) * 0.5;
    }
    return value;
}
...
float MoniqueAudioProcessor::getParameterDefaultValue(int i_)
{
    ...
    {
        value = (1.0f + get_percent_default_modulation_value(
                            automateable_parameters.getUnchecked(i_ - 1))) *
                0.5;
    }
    return value;
}
...
{
    sendParamChangeMessageToListeners(param_->get_info().parameter_host_id,
                                      get_percent_value(param_));
}
...
{
    sendParamChangeMessageToListeners(param_->get_info().parameter_host_id + 1,
                                      (1.0f + param_->get_modulation_amount()) * 0.5);
}
...
double MoniqueAudioProcessor::getTailLengthSeconds() const { return 0.0; }
...
int MoniqueAudioProcessor::getCurrentProgram() { return synth_data->get_current_programm_id_abs(); }
