...
:
    ...
    */
    ...
    float processSingleSampleRaw_noDamp ( float input ) noexcept
    {
        input *= gain;
        float output = 0;

        {
            const float feedbck = feedback.getNextValue();
            for (int j = 0; j < numCombs; ++j)  // accumulate the comb filters in parallel
                output += comb[0][j].process (input, 0, feedbck);
        }

        for (int j = 0; j < numAllPasses; ++j)  // run the allpass filters in series
            output = allPass[0][j].process (output);

        return output * wetGain1.getNextValue() + input * dryGain.getNextValue();
    }
    void processSingleSampleRawStereo_noDamp ( float& input_l, float& input_r ) noexcept
    {
        float outL = 0, outR = 0;
        {
            const float input = (input_l + input_r) * gain;
            const float feedbck = feedback.getNextValue();
            for (int j = 0; j < numCombs; ++j)  // accumulate the comb filters in parallel
            {
                outL += comb[0][j].process (input, 0, feedbck);
                outR += comb[1][j].process (input, 0, feedbck);
            }
        }
        for (int j = 0; j < numAllPasses; ++j)  // run the allpass filters in series
        {
...
private:
    ...
    public:
    CombFilter() noexcept   :
        bufferSize (0), bufferIndex (0), last (0)  {}
