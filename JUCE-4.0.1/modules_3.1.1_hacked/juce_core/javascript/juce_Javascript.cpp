...
struct JavascriptEngine::RootObject   : public DynamicObject
{
    ...
    static bool isNumericOrUndefined (const var& v)  {
        return v.isInt() || v.isDouble() || v.isInt64() || v.isBool() || v.isUndefined();
    }
    ...
    static var* getPropertyPointer (DynamicObject* o, Identifier i)   {
...
CodeLocation (const String& code) noexcept        :
        program (code), location (program.getCharPointer()) {}
CodeLocation (const CodeLocation& other) noexcept :
        program (other.program), location (other.location) {}
    ...
    struct Scope
    {
Scope (const Scope* p, RootObject* r, DynamicObject* s) noexcept :
        parent (p), root (r), scope (s) {}
...
LiteralValue (const CodeLocation& l, const var& v) noexcept :
    ...
    {
UnqualifiedName (const CodeLocation& l, Identifier n) noexcept :
        Expression (l), name (n) {}
    ...
    {
DotOperator (const CodeLocation& l, ExpPtr& p, Identifier c) noexcept :
        Expression (l), parent (p), child (c) {}
...
:
        ...
        virtual var getWithArrayOrObject (const var& a, const var&) const {
            return throwError (a.isArray() ? "Array" : "Object");
        }
...
DivideOp (const CodeLocation& l, ExpPtr& a, ExpPtr& b) noexcept :
        ...
        var getWithInts (int64 a, int64 b) const override       {
            return b != 0 ? var (a / (double) b) : var (std::numeric_limits<double>::infinity());
        }
    };
...
ArrayDeclaration (const CodeLocation& l) noexcept :
    ...
    {
        TokenIterator (const String& code) : location (code), p (code.getCharPointer()) {
            skip();
        }
        ...
        Expression* parseMultiplyDivide()
        {
            ...
            for (;;)
            {
                if (matchIf (TokenTypes::times))        {
                    ExpPtr b (parseUnary());
                    a = new MultiplyOp (location, a, b);
                }
                else if (matchIf (TokenTypes::divide))  {
                    ExpPtr b (parseUnary());
                    a = new DivideOp   (location, a, b);
                }
                else if (matchIf (TokenTypes::modulo))  {
                    ExpPtr b (parseUnary());
                    a = new ModuloOp   (location, a, b);
                }
                else break;
            }
        ...
        Expression* parseAdditionSubtraction()
        {
            ...
            for (;;)
            {
                if (matchIf (TokenTypes::plus))            {
                    ...
                    a = new AdditionOp    (location, a, b);
                }
                else if (matchIf (TokenTypes::minus))      {
                    ExpPtr b (parseMultiplyDivide());
                    a = new SubtractionOp (location, a, b);
                }
                else break;
            }
        ...
        Expression* parseShiftOperator()
        {
            ...
            for (;;)
            {
                if (matchIf (TokenTypes::leftShift))                {
                    ...
                    a = new LeftShiftOp          (location, a, b);
                }
                else if (matchIf (TokenTypes::rightShift))          {
                    ExpPtr b (parseExpression());
                    a = new RightShiftOp         (location, a, b);
                }
                else if (matchIf (TokenTypes::rightShiftUnsigned))  {
                    ExpPtr b (parseExpression());
                    a = new RightShiftUnsignedOp (location, a, b);
                }
                else break;
            }
        ...
        Expression* parseLogicOperator()
        {
            ...
            for (;;)
            {
                if (matchIf (TokenTypes::logicalAnd))       {
                    ...
                    a = new LogicalAndOp (location, a, b);
                }
                else if (matchIf (TokenTypes::logicalOr))   {
                    ExpPtr b (parseComparator());
                    a = new LogicalOrOp  (location, a, b);
                }
                else if (matchIf (TokenTypes::bitwiseAnd))  {
                    ExpPtr b (parseComparator());
                    a = new BitwiseAndOp (location, a, b);
                }
                else if (matchIf (TokenTypes::bitwiseOr))   {
                    ExpPtr b (parseComparator());
                    a = new BitwiseOrOp  (location, a, b);
                }
                else if (matchIf (TokenTypes::bitwiseXor))  {
                    ExpPtr b (parseComparator());
                    a = new BitwiseXorOp (location, a, b);
                }
                else break;
            }
