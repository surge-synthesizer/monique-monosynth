...
public:
    ...
    {
        owner.drawUpDownButton (g, 20, getHeight(),
...
void MidiKeyboardComponent::resetAnyKeysInUse()
{
    ...
    for (int i = mouseDownNotes.size(); --i >= 0;)
    {
        ...
        if (noteDown >= 0)
        {
            state.noteOff (midiChannel, noteDown, 0);
        ...
        if (newNote != oldNoteDown)
        {
            if (oldNoteDown >= 0)
            {
                ...
                if (! mouseDownNotes.contains (oldNoteDown))
                    state.noteOff (midiChannel, oldNoteDown, 0);
            }
    ...
    for (int i = keyPresses.size(); --i >= 0;)
    {
        ...
        else
        {
            if (keysPressed [note])
            {
                ...
                state.noteOff (midiChannel, note, 0);
                keyPressUsed = true;
            }
        }
    }
